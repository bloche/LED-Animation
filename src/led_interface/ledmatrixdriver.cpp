 ...
 */
LEDMatrixDriver::LEDMatrixDriver(char* filename, int typeFlag, QObject *parent) :
    LEDDriver(parent)
{
#ifndef USE_STD_OUT
    #ifdef DEBUG
    ...
    this->currentRow = 0;
    this->globalMask = 1;
    this->globalMaskB = this->globalMask << 1;
    if(typeFlag){
        this->videoReader = new LEDVideoReader(filename,this->buffer, 3072);
    } else {
        this->videoReader = new VideoReader(filename,this->buffer, 3072);
        printf("new videReader created\n");
    }
    this->frameCount = 0;
    this->redLookupTable[0] = 0x00;
    this->redLookupTable[1] = 0x8000;
    this->redLookupTable[2] = 0x8080;
    this->redLookupTable[3] = 0x8410;
    this->redLookupTable[4] = 0x8888;
    this->redLookupTable[5] = 0x9268;
    this->redLookupTable[6] = 0xc630;
    this->redLookupTable[7] = 0x9932;
    this->redLookupTable[8] = 0xaaaa;
    this->redLookupTable[9] = 0e738;
    this->redLookupTable[10] = 0xe79c;
    this->redLookupTable[11] = 0xfc4e;
    this->redLookupTable[12] = 0x7df7;
    this->redLookupTable[13] = 0x7f7f;
    this->redLookupTable[14] = 0x7fff;
    this->redLookupTable[15] = 0xffff;
/*
    this->greenLookupTable[0] = 0x00;
    this->greenLookupTable[1] = 0x00;
    this->greenLookupTable[2] = 0x08;
 ...
 */
void LEDMatrixDriver::inputTest(int input){
  printf("Recieving %i\n",input);
}
void LEDMatrixDriver::outputFrame(uint8_t *frame, int width, int height)
{
    #ifdef DEBUG
    ...
    memcpy(this->buffer, frame, sizeof(uint8_t) * width * height * 3);
}
...
{
   #ifdef DEBUG
 // printf("(LEDMatrixDriver) START output()\n");
  #endif
    if(!this->frameCount){
  //      #ifdef DEBUG
  //      printf("(LEDMatrixDriver) grabbing next frame. random cell is %i\n",this->buffer[452]);
 //       #endif
        this->videoReader->getNextFrame();
    } else {
        if(this->frameCount == 300){
            this->frameCount = -1;
        }
        for(int useless= 0; useless < MAX_USELESS; useless++){}
    }
  //  #ifdef DEBUG
  //  printf("--LEDMatrixDriver::output called %i\n",testCount);
  //  #endif
#ifndef USE_STD_OUT
//int i = 0;
//for(;i< 1;i++){
    ...
    for (column = 0; column < width; column++) {
        ...
        pixel1b = (this->redLookupTable[ pixel1b >> 4] & globalMask) == 0 ? 0 : 1;
        pixel1g = (this->redLookupTable[ pixel1g >> 4] & globalMask) == 0 ? 0 : 1;
        pixel2r = (this->redLookupTable[ pixel2r >> 4] & globalMask) == 0 ? 0 : 1;
        pixel2b = (this->redLookupTable[ pixel2b >> 4] & globalMask) == 0 ? 0 : 1;
        pixel2g = (this->redLookupTable[ pixel2g >> 4] & globalMask) == 0 ? 0 : 1;
...
// I think this is the wrong place for this...
  ...
  //      pixel2b = pixel2b < 128 ? 0 : 0x00;
    ...
    //digitalWrite(LAT, LOW);
    digitalWrite(OE, LOW);
    //printf("Output Finished %i\n", testCount++);
*/
  this->frameCount++;
 // printf("leaving output(), frameCount == %i\n",this->frameCount);
#endif 
}
