...
/* converts in_matirx with in_width columns and in_height rows and returns 
   a matrix three times larger (in_width * in_hight * 3) 
   columns.
                  ...
                  unsigned int wise, unsigned int scan) 
{
  ...
  // allocate memory  
  out_matrix = (uint8_t*) malloc(sizeof(uint8_t) * in_width * in_height * 3);
  ...
  case SCAN_MONODIR:
	...
	for(j=0; j<in_width; j++) {
	  out_matrix[pos++] = in_matrix[i][j].R;
	  out_matrix[pos++] = in_matrix[i][j].G;
	  out_matrix[pos++] = in_matrix[i][j].B;
	}
      break;
	...
	for(j=0; j<in_height; j++) {
	  out_matrix[pos++] = in_matrix[j][i].R;
	  out_matrix[pos++] = in_matrix[j][i].G;
	  out_matrix[pos++] = in_matrix[j][i].B;
	}
      break;
